  ...
  Business.getBusinessByName(req.body.business, function(err, business){
    if(err){
      ...
      for(var m = 0; m < business.users.length; m++){
        if(business.users[m].username === user && business.users[m].role === 'admin'){
          isAdmin = true;
        }
      }
    }
    ...
    if(req.body.status === 'accept'){
        ...
        if(err){
          ...
          Business.getBusinessByName(req.body.business, function(err, business){
            if(err){
              ...
              user.businesses.push(req.body.business);
              business.users.push({id: req.username, role: 'cashier'});
      ...
      User.getUserByUsername(req.username, function(err, user){
        if(err){
          ...
          Business.getBusinessByName(token.business, function(err, business){
            if(err){
              res.sendStatus(500);
            } else {
              ...
              for(var j = 0; j < user.requests.length; j++){
                if(user.requests[i] === token.business){
